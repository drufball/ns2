...
3. Determines the final `SessionStatus`:
   - `stop(complete)` → `SessionStatus::Completed`
   - `stop(waiting)` or no `stop` call → `SessionStatus::Waiting`
4. If a stop signal was received, emits `SessionEvent::Stopped { status, comment }` on the broadcast channel. The `status` field preserves the agent's intent (`Complete` or `Waiting`) so the global issue lifecycle subscriber can act on it (e.g. mark the linked issue `Completed`).
...
- **`complete`** — task is done; the session ends as `Completed` and the linked issue becomes `Completed` (driven by the global issue lifecycle subscriber reacting to `SessionEvent::Stopped`).
