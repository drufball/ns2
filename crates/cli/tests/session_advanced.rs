...
fn write_agent(
    ...
    let ipc = if include_project_config {
        "include_project_config: true\n"
    } else {
        ""
    };
    let hooks = if hooks_yaml.is_empty() {
        String::new()
    } else {
        format!("hooks:\n{hooks_yaml}\n")
    };
...
fn auto_complete_posts_comment_on_issue() {
    ...
    assert!(
        body.contains("\"author\":\"swe\""),
        "missing author in: {body}"
    );
    assert!(
        body.contains("\"body\":\"stub\""),
        "missing stub comment in: {body}"
    );
    assert!(
        body.contains("\"status\":\"completed\""),
        "missing completed status in: {body}"
    );
}
...
fn auto_complete_comment_author_matches_assignee() {
    ...
    let body = h.http_get(&format!("/issues/{id}"));
    assert!(
        body.contains("\"author\":\"qa-tester\""),
        "missing qa-tester author in: {body}"
    );
    assert!(
        body.contains("\"body\":\"stub\""),
        "missing stub comment in: {body}"
...
"#,
    ...
    assert!(
        contents.contains("hook ran"),
        "stop hook did not write expected output"
    );
}
...
"#,
    ...
    let contents = std::fs::read_to_string(&log_path).unwrap();
    assert!(
        contents.contains("project hook ran"),
        "project hook did not write expected output"
    );
}
...
"#,
    ...
    assert!(
        ...
        "exit code should be non-zero on dirty tree"
    );
    let combined = format!(
        "{}{}",
    ...
    assert!(
        ...
        "expected 'uncommitted changes' message"
    );
}
...
"#,
    ...
    assert!(
        !output.status.success(),
        "exit code should be non-zero on staged changes"
    );
    let combined = format!(
        "{}{}",
        String::from_utf8_lossy(&output.stdout),
...
"#,
    ...
    assert!(
        !output.status.success(),
        "exit code should be non-zero with unpushed commits"
    );
    let combined = format!(
        "{}{}",
        String::from_utf8_lossy(&output.stdout),
    ...
    assert!(
        combined.contains("unpushed commit"),
        "expected 'unpushed commit' message, got: {combined}"
    );
}
