...
fn issue_list_shows_all_issues() {
    ...
    assert!(out.contains("assignee"), "list should contain assignee column");
    assert!(out.contains("branch"), "list should contain branch column");
    assert!(out.contains("created_at"), "list should contain created_at column");
}
...
fn issue_list_filter_by_status_open() {
    ...
    assert!(out.contains("Open One"), "filter open should include Open One");
    assert!(out.contains("Open Two"), "filter open should include Open Two");
}
...
fn issue_list_filter_by_assignee() {
    ...
    write_agent(&h, "swe");
    write_agent(&h, "qa");
    h.ns2_stdout(&["issue", "new", "--title", "SWE Task", "--body", "b", "--assignee", "swe"]);
    h.ns2_stdout(&["issue", "new", "--title", "QA Task", "--body", "b", "--assignee", "qa"]);
    h.ns2_stdout(&["issue", "new", "--title", "No Assignee Task", "--body", "b"]);

    let swe_out = h.ns2_stdout(&["issue", "list", "--assignee", "swe"]);
    assert!(swe_out.contains("SWE Task"), "swe filter should include SWE Task");
    assert!(!swe_out.contains("QA Task"), "swe filter should not include QA Task");
    assert!(!swe_out.contains("No Assignee Task"), "swe filter should not include No Assignee Task");

    let qa_out = h.ns2_stdout(&["issue", "list", "--assignee", "qa"]);
    assert!(qa_out.contains("QA Task"), "qa filter should include QA Task");
    assert!(!qa_out.contains("SWE Task"), "qa filter should not include SWE Task");
}
...
fn issue_edit_clear_assignee() {
    ...
    h.start_server();
    write_agent(&h, "swe");
    let id = h.ns2_stdout(&["issue", "new", "--title", "Assigned Task", "--body", "b", "--assignee", "swe"]);
    h.ns2()
...
fn issue_child_inherits_parent_branch() {
    ...
    assert_eq!(parent_branch, child_branch, "child should inherit parent branch");
}
...
fn issue_list_filter_by_parent() {
    ...
    h.ns2_stdout(&["issue", "new", "--title", "Child One", "--body", "b", "--parent", &parent_id]);
    h.ns2_stdout(&["issue", "new", "--title", "Child Two", "--body", "b", "--parent", &parent_id]);
    h.ns2_stdout(&["issue", "new", "--title", "Unrelated", "--body", "b"]);
    let out = h.ns2_stdout(&["issue", "list", "--parent", &parent_id]);
    assert!(out.contains("Child One"), "filter by parent should include Child One");
    assert!(out.contains("Child Two"), "filter by parent should include Child Two");
    assert!(!out.contains("Unrelated"), "filter by parent should not include Unrelated");
    assert!(!out.contains("Parent"), "filter by parent should not include Parent itself");
}
...
fn issue_edit_add_parent() {
    ...
    h.ns2()
        ...
        .success();
    let out = h.ns2_stdout(&["issue", "list", "--parent", &parent_id]);
    assert!(out.contains("Orphan"), "after editing parent, issue should appear in parent filter");
}
...
fn issue_edit_clear_parent() {
    ...
    let parent_id = h.ns2_stdout(&["issue", "new", "--title", "Parent", "--body", "b"]);
    let child_id =
        h.ns2_stdout(&["issue", "new", "--title", "Child", "--body", "b", "--parent", &parent_id]);
    h.ns2()
...
fn issue_edit_replace_blocked_on_list() {
    ...
    h.ns2()
        .args(["issue", "edit", "--id", &blocked_id, "--blocked-on", &blocker2_id])
        .assert()
        .success();
    h.ns2()
    ...
    assert!(out.contains("Blocked Issue"), "issue should still be blocked by blocker2");
}
...
fn issue_start_already_completed_fails() {
    ...
    write_agent(&h, "swe");
    let id =
        h.ns2_stdout(&["issue", "new", "--title", "Done Issue", "--body", "b", "--assignee", "swe"]);
    h.ns2()
...
fn issue_complete_already_completed_fails() {
    ...
    h.ns2()
        ...
        .assert()
        .failure()
        .stderr(predicate::str::contains("terminal state").or(predicate::str::contains("already completed")));
}
...
fn issue_operations_on_nonexistent_id_fail() {
    ...
    h.ns2()
        ...
        .assert()
        .failure()
        .stderr(predicate::str::contains("issue not found: zzzz").or(predicate::str::contains("zzzz")));
}
...
fn issue_new_with_nonexistent_assignee_fails() {
    ...
    h.start_server();
    h.ns2()
        .args(["issue", "new", "--title", "Bad Assignee", "--body", "b", "--assignee", "nonexistent"])
        .assert()
        .failure()
        .stderr(predicate::str::contains("not found in .ns2/agents").or(predicate::str::contains("nonexistent")));
}
...
fn issue_list_shows_branch_column() {
    ...
    let out = h.ns2_stdout(&["issue", "list"]);
    assert!(out.contains("branch"), "list output should contain branch column");
}
