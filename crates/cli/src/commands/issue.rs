...
mod tests {
    ...
    fn attach_snippets_running_node_gets_snippet() {
        ...
        let mut node = make_node_with_session(types::IssueStatus::InProgress, Some(sid), vec![]);
    ...
    fn attach_snippets_recurses_into_children() {
        ...
        let child =
            make_node_with_session(types::IssueStatus::InProgress, Some(child_sid), vec![]);
        let mut root =
    ...
    fn collect_running_sessions_includes_running_nodes() {
        let sid = uuid::Uuid::new_v4();
        let node = make_node_with_session(types::IssueStatus::InProgress, Some(sid), vec![]);
    ...
    fn collect_running_sessions_skips_running_node_without_session_id() {
        let node = make_node_with_session(types::IssueStatus::InProgress, None, vec![]);
