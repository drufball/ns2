...
///
/// A timer fires if the next cron tick after `(now - 60 seconds)` is ≤ `now`.
...
pub fn should_fire(schedule: &str, now: DateTime<Utc>) -> bool {
    ...
    match next_after(schedule, window_start) {
        ...
        Err(e) => {
            tracing::warn!("Timer: invalid schedule '{schedule}': {e}");
            false
        }
    }
}
...
///
/// Checks each timer event's schedule against the 60-second rolling window and,
/// for those that match, emits a `SystemEvent::TimerFired` on the event bus.
pub(crate) async fn process_timer_events(
    ...
    for event in events {
        ...
        let db::EventKind::Timer { ref schedule } = event.kind else {
            continue;
        };
...
mod tests {
    ...
    impl EventStore for StubEventStore {
        async fn create_event(&self, _e: &NamedEvent) -> db::Result<()> {
            Ok(())
        }
        async fn get_event(&self, _id: &str) -> db::Result<NamedEvent> {
            Err(db::Error::NotFound)
        }
        async fn list_events(&self) -> db::Result<Vec<NamedEvent>> {
    ...
    fn make_timer_event(id: &str, name: &str, schedule: &str) -> NamedEvent {
        NamedEvent {
            id: id.into(),
            name: name.into(),
    ...
    // ── process_timer_events integration tests ────────────────────────────────
    ...
    async fn timer_scheduler_fires_event_within_window() {
        ...
        let ev = rx.try_recv().expect("TimerFired event should have been sent");
        match &ev {
            ...
            _ => panic!("expected TimerFired event, got: {ev:?}"),
        }
    }

    #[tokio::test]
    async fn timer_scheduler_does_not_fire_outside_window() {
        let event = make_timer_event("timer-02", "weekly", "0 9 * * 1"); // Monday 9am
        let store: Arc<dyn EventStore> = Arc::new(StubEventStore {
            events: vec![event],
        });
        let bus = EventBus::new(64);
    ...
    async fn process_timer_events_does_not_fire_disabled_event() {
        let mut event = make_timer_event("t3", "heartbeat", "* * * * *");
        event.enabled = false;
        let store: Arc<dyn EventStore> = Arc::new(StubEventStore {
            events: vec![event],
        });
        ...
        assert!(rx.try_recv().is_err(), "disabled timer event must not fire");
    }

    #[tokio::test]
    async fn process_timer_events_skips_webhook_events() {
        // A Webhook kind event should be skipped by the timer scheduler
        let webhook_event = NamedEvent {
            id: "t4".into(),
            name: "ci-hook".into(),
        ...
        assert!(
            rx.try_recv().is_err(),
            "webhook events must not emit TimerFired"
        );
    }

    #[tokio::test]
    async fn process_timer_events_fires_two_events_when_both_match() {
        let ev1 = make_timer_event("t5a", "heartbeat", "* * * * *");
        let ev2 = make_timer_event("t5b", "pulse", "* * * * *");
        let store: Arc<dyn EventStore> = Arc::new(StubEventStore {
            events: vec![ev1, ev2],
        });
        let bus = EventBus::new(64);
    ...
    #[tokio::test]
    async fn timer_fired_contains_event_name() {
        let event = make_timer_event("t-name", "my-timer", "* * * * *");
        let store: Arc<dyn EventStore> = Arc::new(StubEventStore {
            events: vec![event],
        });
        let bus = EventBus::new(64);
        ...
        let ev = rx.try_recv().expect("TimerFired event should have been sent");
        assert!(
            matches!(&ev, SystemEvent::TimerFired { event_name, .. } if event_name == "my-timer"),
            "TimerFired must contain the event_name, got: {ev:?}"
        );
    }
}
