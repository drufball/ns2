...
mod tests {
    ...
    // ─── harness_spawn session-id filter guard tests ──────────────────────────

    /// Helper: create a minimal issue directly in the DB with Running status.
    async fn make_running_issue(db: &Arc<dyn db::Db>, id: &str, title: &str) {
        let now = chrono::Utc::now();
        let issue = types::Issue {
            id: id.to_string(),
    ...
    /// Spawn an issue-watcher task that mirrors the one inside `spawn_harness_sync`.
    /// It subscribes to `event_bus` and filters by `session_id`, calling
    /// `svc.finish_issue` on Done and `svc.fail_issue` on Error.
    fn spawn_issue_watcher(
        event_bus: &EventBus,
        session_id: Uuid,
        issue_id: String,
        svc: issues::IssueService,
            ...
            while let Ok(event) = rx.recv().await {
                match event {
                    events::SystemEvent::Session {
                        ...
                        let _ = svc.finish_issue(&issue_id, None).await;
                        break;
                    }
                    events::SystemEvent::Session {
                        session_id: sid,
                        event: events::SessionEvent::Error { message },
                    } if sid == session_id => {
                        let _ = svc.fail_issue(&issue_id, message).await;
                        break;
                    }
                    _ => {}
                }
    ...
    /// A Done event for session2 must NOT finish issue1 (which is watching session1).
    /// Only when the Done event is for session1 should issue1 become Completed.
    /// issue2 (watching session2) must remain unaffected throughout.
    #[tokio::test]
    async fn test_harness_spawn_done_only_finishes_own_issue() {
        let state = test_state().await;

        // Create two issues, both Running
        make_running_issue(&state.db, "hs-d-issue1", "Issue 1").await;
        make_running_issue(&state.db, "hs-d-issue2", "Issue 2").await;

        let session1_id = Uuid::new_v4();
        let session2_id = Uuid::new_v4();

        // Spawn watchers: issue1 watches session1, issue2 watches session2
        let _watcher1 = spawn_issue_watcher(
            &state.event_bus,
            session1_id,
            "hs-d-issue1".to_string(),
            state.issue_service.clone(),
        );
        let _watcher2 = spawn_issue_watcher(
            &state.event_bus,
            session2_id,
            "hs-d-issue2".to_string(),
            state.issue_service.clone(),
        );
        ...
        // issue1 must still be Running (its watcher did not respond to session2's event)
        let issue1 = state.db.get_issue("hs-d-issue1".to_string()).await.unwrap();
        assert_eq!(
            issue1.status,
            IssueStatus::Running,
        ...
        // Now emit Done for session1 — issue1's watcher should respond
        state.event_bus.send(events::SystemEvent::Session {
        ...
        loop {
            tokio::time::sleep(tokio::time::Duration::from_millis(20)).await;
            let fetched = state.db.get_issue("hs-d-issue1".to_string()).await.unwrap();
            if fetched.status == IssueStatus::Completed {
                break;
            }
            assert!(
        ...
        let issue1_final = state.db.get_issue("hs-d-issue1".to_string()).await.unwrap();
        assert_eq!(
            issue1_final.status,
        ...
        // issue2 became Completed from its own watcher receiving session2's Done event
        // That's expected — but issue1 must not have been the one affected by session2.
        // The key assertion was already checked above (issue1 was still Running after session2 Done).
        // Verify issue2's final state to confirm it processed its own Done correctly.
        let issue2_final = state.db.get_issue("hs-d-issue2".to_string()).await.unwrap();
        assert_eq!(
            issue2_final.status,
            IssueStatus::Completed,
    ...
    /// An Error event for a different session must NOT fail issue1.
    /// Only an Error event for issue1's own session should mark it Failed.
    #[tokio::test]
    async fn test_harness_spawn_error_only_fails_own_issue() {
        let state = test_state().await;

        // Create one issue, Running
        make_running_issue(&state.db, "hs-e-issue1", "Error Filter Issue").await;

        let session1_id = Uuid::new_v4();
        let session2_id = Uuid::new_v4(); // unrelated session

        // Spawn watcher for issue1 → session1
        let _watcher = spawn_issue_watcher(
            &state.event_bus,
            session1_id,
            "hs-e-issue1".to_string(),
            state.issue_service.clone(),
        );
        ...
        state.event_bus.send(events::SystemEvent::Session {
            session_id: session2_id,
            event: events::SessionEvent::Error {
                message: "error from unrelated session".to_string(),
            },
        });
        ...
        let issue1 = state.db.get_issue("hs-e-issue1".to_string()).await.unwrap();
        assert_eq!(
            issue1.status,
            IssueStatus::Running,
        ...
        loop {
            tokio::time::sleep(tokio::time::Duration::from_millis(20)).await;
            let fetched = state.db.get_issue("hs-e-issue1".to_string()).await.unwrap();
        ...
        let issue1_final = state.db.get_issue("hs-e-issue1".to_string()).await.unwrap();
        assert_eq!(
            issue1_final.status,
            IssueStatus::Failed,
