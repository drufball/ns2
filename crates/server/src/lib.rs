            ...
            match rx.recv().await {
                Ok(event) => match event {
                    ...
                    SystemEvent::Issue(IssueEvent::StatusChanged {
                        ...
                        // Resume or start harness
                        let assignee_ok = issue.assignee.is_some();
                        if !assignee_ok {
                            // No assignee — nothing to do (route handler already validated)
                            continue;
                        }

                        // Check if there's an existing Waiting session to resume
                        if let Some(session_id) = issue.session_id {
                            let session_result = state.db.get_session(session_id).await;
                            if let Ok(session) = session_result {
                                if session.status == types::SessionStatus::Waiting {
                                    // Resume: spawn harness against the existing session
                                    let msg_tx = crate::harness_spawn::spawn_harness_sync(
                                        &state, session,
                                    );
                        ...
                        // Otherwise: start fresh (issue is Open after route handler prep)
                        // The route handler will have already called start_issue if this is
                        // a fresh start. We only need to spawn the harness here for the
                        // Waiting→InProgress resume case above.
                        // For the Open→InProgress case the route handler calls start_issue
                        // and then we receive the StatusChanged event AFTER start_issue
                        // creates the session. We need to spawn the harness here.
                        if let Some(session_id) = issue.session_id {
                            let session_result = state.db.get_session(session_id).await;
                            if let Ok(session) = session_result {
                                if session.status == types::SessionStatus::Created {
                                    // Fresh start: spawn harness and send initial message
