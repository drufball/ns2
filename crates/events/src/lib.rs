...
mod tests {
    ...
    fn system_event_external_serde_round_trip() {
        ...
        let decoded: SystemEvent = serde_json::from_str(&json).unwrap();
        assert!(
            matches!(decoded, SystemEvent::External { ref event_id, ref event_name, .. } if event_id == "e001" && event_name == "ci-complete")
        );
    }
    ...
    fn system_event_timer_fired_serde_round_trip() {
        ...
        let decoded: SystemEvent = serde_json::from_str(&json).unwrap();
        assert!(
            matches!(decoded, SystemEvent::TimerFired { ref event_id, ref event_name, .. } if event_id == "t001" && event_name == "heartbeat")
        );
    }
